...
- [ ] TIDE, ECE, FAR analysis, bootstrap significance, Pareto curves — **TIDE and
      bootstrap significance done (2026-08-01, see Decision Log), ECE and Pareto
      still pending.** Both TIDE and bootstrap were run on one representative
      seed per model (closest AP to that model's 3-seed mean), not all 3, since
      they're diagnostic/interpretive rather than the headline metric. Bootstrap
      scoped to the Hidden split only — the only split with a ranking surprise
      actually needing a significance test.
